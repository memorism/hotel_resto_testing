...
class AdminHotelController extends Controller
{
    public function index(Request $request)
    {
        ...
        $monthlyRevenue = $bookings->where('booking_status', 'Not_Canceled')->groupBy('arrival_month')->map(
            fn($row) =>
        ...
        $marketSegmentRevenue = $bookings->where('booking_status', 'Not_Canceled')->groupBy('market_segment_type')->map(
            fn($row) =>
            $row->sum(function ($b) {
        ...
        $roomRevenue = $bookings->where('booking_status', 'Not_Canceled')->groupBy('room_type_reserved')->map(
            fn($row) =>
            $row->sum(fn($b) => ($b->no_of_weekend_nights + $b->no_of_week_nights) * $b->avg_price_per_room)
        )->toArray();
        ...
        $dailyRevenue = $bookings->groupBy(fn($b) => Carbon::createFromDate($b->arrival_year, $b->arrival_month, $b->arrival_date)->format('Y-m-d'))
            ->map(fn($row) => $row->sum(fn($b) => ($b->no_of_weekend_nights + $b->no_of_week_nights) * $b->avg_price_per_room))
            ->toArray();

        $dailyBookings = $bookings->groupBy(fn($b) => Carbon::createFromDate($b->arrival_year, $b->arrival_month, $b->arrival_date)->format('Y-m-d'))
