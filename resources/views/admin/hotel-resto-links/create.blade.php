            ...
            <div class="overflow-hidden bg-white rounded-xl shadow-sm border border-gray-200">
                <div class="p-8">
                    ...
                    <div class="flex items-center justify-between pb-5 mb-6 border-b border-gray-200">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900">Form Tambah Relasi</h3>
                            <p class="mt-1 text-sm text-gray-500">Buat relasi baru antara hotel dan restoran.</p>
                        </div>
                    ...
                    <form action="{{ route('hotel-resto-links.store') }}" method="POST" class="space-y-6">
                        @csrf
                                ...
                                <div class="mt-1">
                                    <select name="resto_id" id="resto_id"
                                        ...
                                        @foreach($restos as $resto)
                                            <option value="{{ $resto->id }}" {{ old('resto_id') == $resto->id ? 'selected' : '' }}>
                                                {{ $resto->name }}
                                            </option>
                                        @endforeach
                                    </select>
