        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-6 border-b border-gray-100">
                    <form method="GET" action="{{ route('scm.rooms.index') }}">
                        <div class="flex items-center gap-3 w-full md:w-1/2">
                            <div class="relative flex-1">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    placeholder="Cari tipe kamar atau deskripsi..."
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($rooms as $index => $room)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $room->total_rooms }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">Rp
                                            {{ number_format($room->price_per_room, 0, ',', '.') }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $room->description }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <x-dropdown-action :edit-url="route('scm.rooms.edit', $room)"
                                            :delete-url="route('scm.rooms.destroy', $room)" />
                                    </td>
                                </tr>
                            @empty
                                <tr>
